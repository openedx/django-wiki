...
class Edit(ArticleMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        revision.set_from_request(self.request)
        self.article.add_revision(revision)
        messages.success(self.request, _('A new revision of the article was successfully added.'))
        return self.get_success_url()
...
def change_revision(request, article, revision_id=None, urlpath=None):
    ...
    article.save()
    messages.success(request, _('The article %(title)s is now set to display revision #%(revision_number)d') % {
...
class CreateRootView(FormView):
    ...
    template_name = 'wiki/article/create_root.html'
